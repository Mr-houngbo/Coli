  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error('Erreur chargement Expéditeurs:', e?.message || e);
        setError("Impossible de charger les expéditeurs.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Expéditeurs</h1>
        {items.length === 0 ? (
          <div className="text-center py-12 bg-white border border-gray-200 rounded-lg">Aucune demande d'expéditeur</div>
        ) : (
